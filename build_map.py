#!/usr/bin/env python3
"""
Build script for Singapore Condo Launches 2026 Map
Extracts data, geocodes locations, and generates a single-file HTML map.
"""
...
# Extracted data from the image table
RAW_DATA = [
    {
    ...
    # EC (Executive Condominium) Launches
    {
...
# Type data from Type.png - maps project/location to Type, Sold In, S$ psf ppr
TYPE_DATA = {
    "Holland Link": {"type": "GLS", "sold_in": "Jul-25", "psf_ppr": "$1,432"},
    "River Modern": {"type": "GLS", "sold_in": "Feb-25", "psf_ppr": "$1,420"},
    "Dunearn Road": {"type": "GLS", "sold_in": "Jun-25", "psf_ppr": "$1,410"},
    "Bayshore Road": {"type": "GLS", "sold_in": "Mar-25", "psf_ppr": "$1,388"},
    "Chuan Grove": {"type": "GLS", "sold_in": "Sept-25", "psf_ppr": "$1,376"},
    "Dorset Road": {"type": "GLS", "sold_in": "Oct-25", "psf_ppr": "$1,338"},
    "Thomson View": {"type": "Enbloc", "sold_in": "Oct-24", "psf_ppr": "$1,178"},
    "Former Thomson View condo": {"type": "Enbloc", "sold_in": "Oct-24", "psf_ppr": "$1,178"},
    "Sophia Meadows": {"type": "Enbloc", "sold_in": "Jan-24", "psf_ppr": "$1,172"},
    "Lakeside Drive": {"type": "GLS", "sold_in": "Jun-25", "psf_ppr": "$1,132"},
    "Duet @ Emily": {"type": "Enbloc", "sold_in": "Mar-21", "psf_ppr": "$1,115"},
    "Upper Thomson Road": {"type": "GLS", "sold_in": "Oct-25", "psf_ppr": "$1,062"},
    "Upper Thomson Road (Parcel A)": {"type": "GLS", "sold_in": "Oct-25", "psf_ppr": "$1,062"},
    "Media Circle": {"type": "GLS", "sold_in": "Mar-25", "psf_ppr": "$1,037"},
    "Media Circle (Parcel A)": {"type": "GLS", "sold_in": "Mar-25", "psf_ppr": "$1,037"},
    "Narra Residences": {"type": "GLS", "sold_in": "Jan-25", "psf_ppr": "$1,020"},
    "Pinery Residences": {"type": "GLS", "sold_in": "Sept-24", "psf_ppr": "$1,004"},
    "Chencharu Close": {"type": "GLS", "sold_in": "Sept-25", "psf_ppr": "$980"},
    "Lentor Gardens": {"type": "GLS", "sold_in": "Apr-25", "psf_ppr": "$920"},
    "Tengah Garden Avenue": {"type": "GLS", "sold_in": "Jan-25", "psf_ppr": "$821"},
    "Tengah Garden Ave": {"type": "GLS", "sold_in": "Jan-25", "psf_ppr": "$821"},
    "Newport Residences": {"type": "Redevelopment", "sold_in": "-", "psf_ppr": "-"},
    "Woodlands Drive": {"type": "GLS", "sold_in": "Oct-25", "psf_ppr": "$782"},
    "Senja Close": {"type": "GLS", "sold_in": "Aug-25", "psf_ppr": "$771"},
    "Rivelle Tampines": {"type": "GLS", "sold_in": "Oct-24", "psf_ppr": "$768"},
    "Coastal Cabana": {"type": "GLS", "sold_in": "Aug-24", "psf_ppr": "$705"},
    "Sembawang Road": {"type": "GLS", "sold_in": "Sept-25", "psf_ppr": "$692"},
}

# Manual fallback coordinates for locations - researched accurate Singapore coordinates
# Based on actual street/road positions in Singapore
FALLBACK_COORDS = {
    # D23 Dairy Farm - near Dairy Farm MRT, Bukit Panjang area
    "Dairy Farm Walk": (1.3648, 103.7717),
    # D2 Anson Road - CBD area, near Tanjong Pagar
    "Anson Road": (1.2755, 103.8440),
    # D9 Emily Road - Orchard/Dhoby Ghaut area
    "Emily Road": (1.2995, 103.8485),
    # D9 Sophia Road - near Dhoby Ghaut
    "Sophia Road": (1.3015, 103.8480),
    # D18 Tampines Street 94 - Tampines area
    "Tampines Street 94": (1.3545, 103.9565),
    # D24 Tengah Garden Avenue - new Tengah town
    "Tengah Garden Avenue": (1.3635, 103.7175),
    # D9 River Valley Green - near River Valley Road
    "River Valley Green": (1.2945, 103.8350),
    # D15 Bayshore Road - East Coast area
    "Bayshore Road": (1.3055, 103.9385),
    # D5 Media Circle - one-north/Buona Vista area (Mediapolis)
    "Media Circle": (1.2975, 103.7885),
    # D26 Lentor Gardens - Lentor area, near Lentor MRT
    "Lentor Gardens": (1.4125, 103.8375),
    # D11 Dunearn Road - Bukit Timah area
    "Dunearn Road": (1.3245, 103.8095),
    # D10 Holland Link - Holland Village area
    "Holland Link": (1.3105, 103.7895),
    # D22 Lakeside Drive - Jurong Lake area
    "Lakeside Drive": (1.3395, 103.7205),
    # D27 Chencharu Close - Yishun/Sembawang area
    "Chencharu Close": (1.4235, 103.8125),
    # D19 Chuan Grove - Serangoon/Lorong Chuan area
    "Chuan Grove": (1.3510, 103.8670),
    # D8 Dorset Road - Little India/Farrer Park area
    "Dorset Road": (1.3125, 103.8565),
    # D20 Bright Hill Drive - near Bishan, Thomson area
    "Bright Hill Drive": (1.3620, 103.8355),
    # D26 Upper Thomson Road - near Springleaf area
    "Upper Thomson Road": (1.4025, 103.8365),
    # EC Locations
    # D17 Jalan Loyang Besar - Pasir Ris/Loyang area
    "Jalan Loyang Besar": (1.3725, 103.9620),
    # D18 Tampines Street 95 - Tampines area
    "Tampines Street 95": (1.3520, 103.9545),
    # D25 Woodlands Drive - Woodlands area
    "Woodlands Drive": (1.4350, 103.7865),
    # D27 Sembawang Road - Sembawang area
    "Sembawang Road": (1.4420, 103.8205),
    # D23 Senja Close - Bukit Panjang area
    "Senja Close": (1.3785, 103.7625),
}
...
def geocode_location(query, cache):
    ...
    req = urllib.request.Request(
        url,
        headers={'User-Agent': 'CondoLaunches2026Map/1.0 (contact@example.com)'}
    )
...
def geocode_with_fallback(item, cache):
    """
    Try multiple geocoding strategies:
    1. Location + Singapore
    2. Project + Singapore
    3. Location + Singapore + District
    4. Manual fallback coordinates
    """
    location = item['location']
    project = item['project']
    district = item['district']

    # Strategy 1: Location + Singapore
    query1 = f"{location}, Singapore"
    result = geocode_location(query1, cache)
    if result:
        return result

    # Strategy 2: Project + Singapore
    query2 = f"{project}, Singapore"
    result = geocode_location(query2, cache)
    if result:
        return result

    # Strategy 3: Location + Singapore + District
    query3 = f"{location} Singapore District {district}"
    result = geocode_location(query3, cache)
    if result:
        return result

    # Strategy 4: Manual fallback
    if location in FALLBACK_COORDS:
        print(f"  [FALLBACK] Using manual coordinates for {location}")
        coords = FALLBACK_COORDS[location]
        cache[query1] = coords  # Cache for future runs
        save_cache(cache)
        return coords

    print(f"  [WARNING] Could not geocode {location} - using Singapore center")
    return (1.3521, 103.8198)  # Singapore center as last resort
...
<head>
    ...
    <title>Singapore Condo Launches 2026 | New Property Launch Map</title>

    <!-- SEO Meta Tags -->
    <meta name="description" content="Interactive map of upcoming Singapore condo and EC launches in 2026. View 23 new property developments across all districts with details on developer, units, tenure, and estimated launch dates.">
    <meta name="keywords" content="Singapore condo launch 2026, new condo Singapore, property launch 2026, EC launch Singapore, Singapore property map, new development Singapore, condo launch dates">
    <meta name="author" content="Singapore Property Launches">
    <meta name="robots" content="index, follow">

    <!-- Open Graph / Facebook -->
    <meta property="og:type" content="website">
    <meta property="og:title" content="Singapore Condo Launches 2026 | Interactive Property Map">
    <meta property="og:description" content="Explore 23 upcoming condo and EC launches in Singapore for 2026. Interactive map with launch dates, developer info, and unit counts.">
    <meta property="og:locale" content="en_SG">

    <!-- Twitter Card -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="Singapore Condo Launches 2026">
    <meta name="twitter:description" content="Interactive map of 23 upcoming Singapore condo and EC launches in 2026.">
    ...
    {{
        ...
        "@type": "WebApplication",
        "name": "Singapore Condo Launches 2026",
        "description": "Interactive map of upcoming Singapore condo and EC launches in 2026",
        ...
        "author": {{
            "@type": "Organization",
            "name": "Singapore Property Launches"
        }}
    }}
    </script>
...
def main():
    print("=" * 60)
    print("Singapore Condo Launches 2026 Map Builder")
    print("=" * 60)

    # Step 1: Load cache
    print("\n[1/4] Loading geocode cache...")
    cache = load_cache()
    print(f"     Cache has {len(cache)} entries")

    # Step 2: Geocode all locations
    print("\n[2/4] Geocoding locations...")
    data = []
    for item in RAW_DATA:
        print(f"\n  Processing: {item['project']}")
        lat, lng = geocode_with_fallback(item, cache)

        # Create final data item with coordinates
        final_item = {**item, 'lat': lat, 'lng': lng}

        # Add type data if available (match by project name or location)
        type_info = TYPE_DATA.get(item['project']) or TYPE_DATA.get(item['location']) or {}
        final_item['site_type'] = type_info.get('type', '-')
        final_item['sold_in'] = type_info.get('sold_in', '-')
        final_item['psf_ppr'] = type_info.get('psf_ppr', '-')

        data.append(final_item)
        print(f"    -> ({lat:.4f}, {lng:.4f})")

    # Step 3: Save data JSON
    print("\n[3/4] Saving data JSON...")
    ...
    print("=" * 60)
    print(f"\nTotal condos: {len(data)}")
    print(f"Unique launch periods: {len(set(item['launch_period'] for item in data))}")
    print(f"\nFiles created:")
    print(f"  - {HTML_FILE}")
    print(f"  - {DATA_FILE}")
    print(f"  - {CACHE_FILE}")
    print(f"\nOpen {HTML_FILE.name} in your browser to view the map!")
